...
- `feat/backend-core`：共享后端、模型、路由、SQL、测试基线。涉及公共数据结构时只能在这个分支改。
- `feat/student-ui`：学生端模板、样式、前端交互细节，可在不修改共享模型和路由协议的前提下并行开发。
- `feat/admin-screen`：后台模板、大屏模板、图表表现层，可在不改共享统计接口契约的前提下并行开发。
- `feat/report-docs`：部署说明、数据库说明、测试记录、报告提纲、素材清单，可与编码并行推进。
- 不能并行的情况：
  - 多个分支同时修改同一份共享 Python 路由或模型。
  - 多个分支同时改同一份 SQL 结构文件。
  - 一个分支依赖另一个分支尚未确定的接口字段。
